...
def valid_test_DataSource() -> pd.DataFrame:
    ...
    data = FakeWeatherDataGenerator(
        location_no=LOCATION_NO, timepoint_no=TIMEPOINT_NO
    ).produce_data()
...
def test_DataSource_secondary_measures_list(valid_test_DataSource):
    ...
    assert sorted(meas_list) == sorted(
        [
            "cloudcover",
            "lifted_index",
            "seeing",
...
def test_DataSource_filter(valid_test_DataSource):
    ...
    filtered = valid_test_DataSource.filter(location=random_location, measures=measures)
    assert filtered.row_count == 10
    assert filtered.data.shape[1] == 8
    assert sorted(filtered.secondary_measures_list) == sorted(
        ["transparency", "lifted_index", "wind10m_speed"]
    )
    filtered = valid_test_DataSource.filter(location=random_location, measures=None)
    assert filtered.row_count == 10
...
def test_DataSource_prepare_data_table(valid_test_DataSource):
    ...
    assert valid_test_DataSource.build_data_table(sort_by="timepoint")[
        "timepoint"
